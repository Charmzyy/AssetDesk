...
| CSV/XLSX import | `src/frontend/CsvImportPanel.jsx` | Agents, on an issue | `jira:issuePanel` |
...
## 4. CSV/XLSX import panel — `CsvImportPanel.jsx`

Agent-facing panel rendered on every issue view. Two sections:

### Automatic import plan (top section)

The panel face of the automated flow (`importPostFunctions.js` — see
`backend.md` § 5b). State: `plan` mirrors the issue's `csv-import-plan:<issueId>`
KVS record; `pollPlan` re-fetches it every 1.5 s while an import is running
(including one started elsewhere, e.g. by the approve post-function — the panel
resumes polling on mount if it loads a plan already in `importing`).

- **Analyze newest attachment** → `analyzeImportPlan`: builds the plan from the
  newest CSV/XLSX attachment; one card per file/sheet showing the object type
  detected from its *name*, row count, matched-column count, and the
  first-column unique-key check.
- **Per-unit object-type `Select`** → `overrideImportPlanUnit`: the escape hatch
  for units the name detector left unresolved (ambiguous, abstract type, no
  match) or resolved to a parent type when a child was meant (`isParentType`
  warning). Manual choices are marked `matchedBy: 'manual'`.
- **Start import / Run again** → `confirmImportPlan` (with its own create-only
  Toggle): starts the same chained per-sheet run the approve post-function
  starts; per-unit results fill in on each card as the chain progresses.

### Manual single-CSV import (bottom section — the original wizard, unchanged)
