...
| Scopes are declared in `manifest.yml` and consented at install | The app holds read/write CMDB scopes, `storage:app`, `read:attachment:jira`, a few Jira read scopes, and `write:jira-work` (solely for the automated import flow's plan/result issue comments). Adding a scope requires redeploy **and** reinstall. |
...
| Spreadsheets | `xlsx` (SheetJS) | XLSX export **and** CSV parsing **and** Excel date-serial decoding **and** splitting import workbooks into per-sheet CSV units |
...
flowchart LR
    ...
    subgraph WF["workflow modules (jira:workflowPostFunction)"]
        W1["AssetDesk — Analyze asset import file<br/>import-analyze-fn"]
        W2["AssetDesk — Run approved asset import<br/>import-approve-fn"]
    end
...
- The two **workflow post-functions** are placed on transitions by a Jira admin
  (typically: "Analyze" on the transition into an awaiting-approval status,
  "Run" on the approve transition). They run `asApp()` with no user session —
  authorization is "who may execute the transition." Both are backed by
  `src/resolvers/importPostFunctions.js`; see § 8 for the flow.
...
| `csv-import-plan:<issueId>` | analyze post-function / panel Analyze, updated by `advanceImportPlan` | approve post-function, panel plan section | The per-**issue** automated-import plan: which attachment, one "unit" per CSV file / XLSX sheet with its detected object type, column/key validation, and (during a run) per-unit results. Metadata only — headers capped at 50, candidates 10, unmatched columns 30; row data is never stored. Overwritten by re-analysis. |
...
## 8. Data flow: CSV/XLSX import

Two entry paths share the same queue consumer: the original **manual** panel flow
(one CSV, agent picks the object type) and the **automated** flow (workflow
post-functions + the panel's "Automatic import plan" section — CSV *or* XLSX,
object types detected from file/sheet names).

### 8a. Manual flow (panel, CSV only)
...
### 8b. Automated flow (workflow post-functions, CSV or XLSX)

```mermaid
flowchart TD
    A["Agent attaches file to ticket<br/>(laptops.csv, or inventory.xlsx<br/>with sheets named per type)"] --> B["Transition with ANALYZE post-function<br/>(or panel 'Analyze newest attachment')"]
    B --> C["buildImportPlanForIssue (asApp):<br/>newest CSV/XLSX → extractImportUnits<br/>(1 unit per CSV file / non-empty sheet)"]
    C --> D["per unit: detectObjectTypeFromName<br/>token match on file/sheet name,<br/>longest wins, ties = ambiguous"]
    D --> E["validate columns + first-column<br/>unique key against detected type"]
    E --> F["plan → KVS csv-import-plan:issueId<br/>+ plan COMMENT on the issue<br/>(nothing imported yet)"]
    F --> G["Human checkpoint:<br/>read comment; fix unresolved units<br/>via panel dropdowns if needed"]
    G --> H["Transition with RUN post-function<br/>(or panel 'Start import')"]
    H --> I["startImportPlanJobs:<br/>enqueue FIRST runnable unit only"]
    I --> J["csvImportConsumer<br/>(same per-row pipeline as 8a;<br/>XLSX sheet → sheet_to_csv first)"]
    J --> K["finalizeJob → advanceImportPlan:<br/>record unit result, enqueue NEXT unit"]
    K -- "more units" --> J
    K -- "last unit" --> L["plan status 'done' +<br/>per-sheet summary COMMENT"]
```

Design decisions worth knowing:

- **Detection is name-based only** — `tv_stock_july.csv` → type "TV"; a sheet
  "new laptops 2026" → "Laptops". Header-based auto-detection was deliberately
  left out: with attribute inheritance (parent types whose children share most
  attributes), sibling types are indistinguishable by headers. Ambiguity is never
  guessed — unresolved units are skipped and reported, resolvable in the panel.
- **Confirmation is workflow-native.** Analyze writes a plan and a comment;
  nothing imports until someone executes the approve transition (or the panel
  Confirm) — so "who may import" is exactly "who may run that transition."
- **Units run sequentially, chained through the queue** (not pushed all at once):
  queue delivery has no ordering guarantee, and workbook order matters —
  reference targets (e.g. a Manufacturers sheet) must import before referrers
  (a Laptops sheet whose "Manufacturer" column is looked up by name). Each sheet
  also gets its own 900 s consumer budget.
- **Issue comments are the flow's only feedback channel** (plan, start, summary)
  — hence the `write:jira-work` scope. The panel's plan section shows the same
  state live for whoever has it open.
